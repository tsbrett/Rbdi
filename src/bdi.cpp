...
double P_BDC(double x, double x0,
              double lambda, double mu, double t, double eps){
...
double P_BDIC(double x, double x0, double v,
  ...
  if(x0 == 0){
    ...
    for (int i = 0; i <= x; i++){
      ...
      prob_BD = P_BDC(x-i, x0, lambda, mu, t,eps=eps);
...
double P_BDIC_old(double x, double x0, double v,
	...
	// double p_BDI = m_BDI/(m_BDI+v); // this is not quite 1 - p
	// double R0 = lambda/mu; // yes but unused
	// double rho = A_BD/mu;
	...
	for (int i = 0; i <= x; i++)
	{
		...
		{
			//Do I need to round?
			min_x = std::min(x-i,x0);
			for (int j = 0; j <= min_x; j++ )
			{
				prob_BD += c_choose(x0,j)*c_choose(x0 + (x-i) -j - 1, x0 -1)* //tick
				pow(A_BD,(x0-j))* //tick
				pow(B_BD,((x-i)-j))* //tick
				pow((1.0 - A_BD-B_BD),j); //tick
			}
...
double C_bdi_ll_linear_R0(NumericVector x, double delta_t,
	...
	for(int m =1; m < n; m++){ // seems correct
	  ...
	  p_m = P_BDIC(x[m], x[m-1], eta, lambda, gamm, delta_t, eps);
